    ...
    async processChannelForSync(channel, modelMapping, modelUpdateMode = 'append') {
        ...
        if (channelModelMapping && Object.keys(channelModelMapping).length > 0) {
            ...
            for (const [originalName, newName] of Object.entries(channelModelMapping)) {
                ...
                // 只在渠道包含目标模型时应用映射，避免跨渠道误匹配
                const foundTarget = originalModelSet.has(targetName);
                console.log(`🔍 检查目标模型名 "${targetName}" 是否在渠道中: ${foundTarget}`);

                if (foundTarget) {
                    // 构建 NewAPI 需要的格式: { 新模型名: 原始模型名 }
                    relevantMapping[targetName] = sourceName;
                    console.log(`✅ 添加映射: ${targetName} → ${sourceName}`);
                } else {
                    console.log(`⏭️ 跳过映射: ${targetName} (不在渠道中)`);
                }
            }
