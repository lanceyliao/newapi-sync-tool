  ...
  parseModels(modelsData) {
    ...
    async processChannelForSync(channel, modelMapping, modelUpdateMode = 'append') {
        ...
        if (channelModelMapping && Object.keys(channelModelMapping).length > 0) {
            ...
            for (const [originalName, newName] of Object.entries(channelModelMapping)) {
                // 检查原始模型是否存在于当前渠道
                // 注意：追加模式下，即使原始模型不在列表中，也应该添加映射
                // 因为之前可能用覆盖模式同步过，导致原始模型被移除
                const found = originalModels.includes(originalName);
                console.log(`🔍 检查模型 "${originalName}" 是否在渠道中: ${found}`);

                // 无论是否找到，都添加映射（追加模式的核心逻辑）
                // 构建 NewAPI 需要的格式: { 新模型名: 原始模型名 }
                relevantMapping[newName] = originalName;
            }
        ...
        // 在追加模式下，即使没有新的映射变化，也需要检查是否需要保留现有映射
